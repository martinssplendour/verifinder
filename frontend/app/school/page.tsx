...
function SchoolCard({ result }: { result: SchoolSearchResult }) {
  return (
    <Link href={`/school/${result.urn}`} className="result-card school-result-card">
      <span className="result-icon school-result-icon"><School size={22} /></span>
      <div className="result-title">
        <h2>{result.establishment_name}</h2>
        <p>{result.type_name || "Establishment"} · {result.phase_name || "Phase unavailable"} · {result.status_name || "Status unavailable"}</p>
      </div>
      <div className="result-meta">
        <span><MapPin size={14} /> {result.postcode || result.town || "Location unavailable"}</span>
        <span>URN {result.urn}</span>
      </div>
      <span className="result-open">View school <ArrowRight size={16} /></span>
    </Link>
  );
}
  ...
  return (
    <div className="shell search-page engine-search-page school-search-page">
      ...
      {!query || query.length < 2 ? (
        <div className="empty-state"><SearchX size={28} /><h2>Enter a school name, URN or postcode</h2><p>URN is the Department for Education's unique reference number for the establishment.</p></div>
      ) : error ? (
        <div className="empty-state error-state" role="alert"><CircleAlert size={28} /><h2>School Check is unavailable</h2><p>{error}</p></div>
      ) : !current ? (
        ...
        <section className="result-group">
          ...
          <div className="result-list">{current.results.map((result) => <SchoolCard result={result} key={result.urn} />)}</div>
        </section>
      )}
    </div>
  );
