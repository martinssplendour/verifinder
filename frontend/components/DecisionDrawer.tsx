      ...
      {mode && <div className="decision-drawer-layer">
        ...
        <section className="decision-drawer" role="dialog" aria-modal="true" aria-labelledby="decision-drawer-title">
          ...
          <div className="drawer-conversation">
            {messages.map((item) => <div className={`chat-row chat-${item.role}`} key={item.id}>{item.role === "assistant" && <span className="chat-avatar"><Bot size={15} /></span>}<div className="chat-content"><p>{item.text}</p>{item.ask && <AskAnswer answer={item.ask} close={close} ask={(question) => void ask.submit(question)} />}</div></div>)}
