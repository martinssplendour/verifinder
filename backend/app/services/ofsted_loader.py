...
from app.models import DataSource, DatasetVersion, IngestionRun, OfstedInspectionRecord, RunStatus, SourceHealth
from app.services.dataset_utils import chunks
from app.services.ofsted_ingestion import OfstedSnapshot, ofsted_rows


SOURCE_ID = "ofsted-school-inspections"
OFFICIAL_URL = "https://www.gov.uk/government/statistical-data-sets/monthly-management-information-ofsteds-school-inspections-outcomes"
DATA_URL = "https://www.gov.uk/government/statistical-data-sets/monthly-management-information-ofsteds-school-inspections-outcomes"
...
def _ensure_source(session, retrieved_at: datetime) -> DataSource:
    ...
    if source is None:
        source = DataSource(
            id=SOURCE_ID,
            organisation="Ofsted",
            name="State-funded school inspections and outcomes: management information",
            source_type="CSV",
...
def load_ofsted_snapshot(snapshot: OfstedSnapshot, retrieved_at: datetime, published_on: date | None = None) -> dict:
    ...
    try:
        ...
        inserted = 0
        for batch in chunks(ofsted_rows(snapshot, version.id), size=2_000):
            session.execute(insert(OfstedInspectionRecord), batch)
            inserted += len(batch)
