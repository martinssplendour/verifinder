...
from app.models import DataSource, DatasetVersion, FoodEstablishmentRecord, IngestionRun, RunStatus, SourceHealth
from app.services.dataset_utils import chunks
from app.services.food_ingestion import FoodSnapshot, food_rows


SOURCE_ID = "fsa-food-hygiene"
OFFICIAL_URL = "https://ratings.food.gov.uk/open-data"
DATA_URL = "https://safhrsprodstorage.blob.core.windows.net/opendatafileblobstorage/FHRS_All_en-GB.csv"
...
def load_food_snapshot(snapshot: FoodSnapshot, retrieved_at: datetime, published_on: date | None = None) -> dict:
    ...
    try:
        ...
        inserted = 0
        for batch in chunks(food_rows(snapshot, version.id), size=5_000):
            session.execute(insert(FoodEstablishmentRecord), batch)
            inserted += len(batch)
